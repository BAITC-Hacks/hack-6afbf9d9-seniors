...
import { civicText } from './civic.js';
import { renderCivicReputation, renderCivicFinale } from './civic-view.js';
...
export function renderHQMap({ data, evaluation, story, language, icon, num, signed, affected = [], full = false }) {
  ...
  const civic = key => esc(civicText(key, language));
  const stateLabel = state => civic({ critical: 'mapCritical', improving: 'mapImproving', stable: 'mapStable' }[state]);
  const legend = `<div class="civic-map-legend"><strong>${civic('mapLegend')}</strong><ul>${['critical', 'improving', 'stable'].map(state => `<li data-state="${state}"><i aria-hidden="true"></i>${stateLabel(state)}</li>`).join('')}</ul><details><summary>${civic('mapCategories')}</summary><ul>${data.categories.map(category => `<li data-category="${esc(category.id)}"><i aria-hidden="true"></i>${tr(category.name)}</li>`).join('')}</ul></details></div>`;
  ...
  return `<section class="hq-map-panel ${full ? 'hq-map-full' : ''}" aria-labelledby="hq-map-title"><div class="hq-panel-heading"><h2 id="hq-map-title">${d('mapTitle')}</h2><span>${d('future')}</span></div><div class="hq-city-map"><img src="/city-map.svg" alt=""/>${routes}${data.districts.map(district => {
    ...
    const condition = Object.values(result.metrics).some(value => Number.isFinite(value) && value < 40) ? 'critical' : result.delta > 0 ? 'improving' : 'stable';
    return `<button class="hq-map-district ${selected.id === district.id ? 'selected' : ''} ${affected.includes(district.id) || districtCategories.length ? 'is-affected' : ''}" style="--map-x:${x}%;--map-y:${y}%" data-action="story-map-district" data-id="${esc(district.id)}" data-state="${condition}" data-category="${esc(districtCategories[districtCategories.length - 1] || '')}" data-categories="${esc(districtCategories.join(' '))}" aria-pressed="${selected.id === district.id}" aria-label="${tr(district.name)}: ${num(result.before)} → ${num(result.after)}. ${stateLabel(condition)}"><i class="hq-map-dot" aria-hidden="true"></i><span><strong>${tr(district.name)}</strong><small>${num(result.after)} <em>${signed(result.delta)}</em></small><span class="hq-map-category-dots" aria-hidden="true">${districtCategories.map(category => `<i data-category="${esc(category)}"></i>`).join('')}</span></span></button>`;
  }).join('')}</div>${legend}${transport.length ? `<p class="hq-map-route-caption">${d('schematicRoutes')}</p>` : ''}<div class="hq-district-summary"><h3>${icon('pin')}${tr(selected.name)}</h3><div class="hq-before-after"><span>${d('before')} <b>${num(details.before)}</b></span>${icon('arrow')}<span>${d('forecast')} <b>${num(details.after)}</b></span></div><details class="hq-indicator-details"><summary>${d('districtDetails')}</summary>${data.indicators.map(indicator => `<div class="hq-indicator-row"><span>${tr(indicator.name)}</span><b>${num(baseline.metrics[indicator.id])} → ${num(details.metrics[indicator.id])}</b></div>`).join('')}</details></div></section>`;
}
...
export function renderHQFinale({ data, evaluation, story, language, icon, num, signed, button }) {
  ...
  return `<div class="hq-finale hq-ending-${esc(summary.style.id)}"><section class="hq-final-hero"><div class="hq-final-address"><div class="hq-chapter">18:40 · ${d('address')}</div><h1>${esc(summary.style.title)}</h1>${summary.address.map(line => `<p>${highlightDialogue(line, language)}</p>`).join('')}</div><div class="hq-final-score"><span>ASTANA QUALITY OF LIFE SCORE</span><strong>${num(evaluation.score)}</strong><p>${num(evaluation.baselineScore)} → ${num(evaluation.score)} <b>${signed(evaluation.delta)}</b></p><small>${d('future')}</small></div></section><div class="hq-final-map">${renderHQMap({ data, evaluation, story, language, icon, num, signed, full: true })}</div><div class="hq-final-grid"><section class="hq-achievements"><h2>${d('achievements')}</h2>${summary.achievements.map(item => `<article class="hq-achievement">${icon('check')}<div><h3>${esc(item.label)}</h3><p>${esc(item.detail)}</p></div></article>`).join('')}</section><section class="hq-problems"><h2>${d('problems')}</h2>${summary.problems.map(item => `<article class="hq-problem">${icon('info')}<div><h3>${esc(item.label)}</h3><p>${esc(item.detail)}</p></div></article>`).join('')}</section></div><section class="hq-playstyle"><div class="hq-chapter">${d('style')}</div><h2>${esc(summary.style.title)}</h2><p>${esc(summary.style.body)}</p><ul>${summary.style.reasons.map(reason => `<li>${esc(reason)}</li>`).join('')}</ul></section>${renderCivicFinale({ data, evaluation, story, language, icon, num, signed })}<div class="story-button-row hq-final-actions">${button('story-replay', d('replay'), 'clock', 'story-primary')}${button('story-analyze', t('analyze'), 'sparkle')}</div></div>`;
}
