...
1. Откройте симулятор: бюджет **100**, базовый Score **52,56**, решений **0/5**.
...
Этот необязательный тест проверяет JavaScript, генерацию всех экранов, загрузку примера, экспорт, повреждённую историю, запоздавший AI-ответ и отказ сервера. Он не запускает браузер и не проверяет визуальную вёрстку. Запросы к платному AI в тестах не выполняются.
